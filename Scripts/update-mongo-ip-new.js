...
const LOG_FILE = path.join(__dirname, 'ip-update.log');
...
async function addIPToWhitelist(ip, comment = 'Added automatically by IP updater script') {
  try {
...
// Main function
async function main() {
  log('Starting MongoDB Atlas IP whitelist update...');
  ...
  if (!currentIP) {
    log('Could not determine current IP address. Exiting.');
    return;
  }
  ...
  if (currentIP === cachedIP) {
    ...
    // Get current whitelisted IPs to display
    ...
    whitelistedIPs.forEach(entry => {
      log(`- ${entry.ipAddress} (${entry.comment || 'No comment'})`);
    });
  ...
  if (success) {
    ...
    log('Failed to update IP in MongoDB Atlas whitelist.');
  }
}
